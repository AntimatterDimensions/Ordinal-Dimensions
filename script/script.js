...
function buyAutoIncr(num) {
  if (game.ord >= game.autoIncrCost[num]) {
    ...
    game.autoIncrCost[num] = (autoIncrCostBase[num]*(10**(num+1))**game.autoIncrBought[num]).toExponential(0);
  }
}
...
function loop(unadjusted, off = 0) {
  ...
  for (var i = 9; i > 0; i--) {
    ...
    game.autoIncrCost[i] = (autoIncrCostBase[i]*(10**(i+1))**game.autoIncrBought[i]).toExponential(0);
  }
  game.ord += game.autoIncrHave[0]*ms/1000*2**game.autoIncrBought[0];
  game.autoIncrCost[i] = (autoIncrCostBase[0]*(10**(0+1))**game.autoIncrBought[0]).toExponential(0);
